        ...
        while (weakSelf.isPlay && weakSelf.lastCount > 0) {
            ...
            if ((weakSelf.isSourceChange || src == nil) && weakSelf.gifData != nil) {
                ...
                if (src) {
                    ...
                    frameDelayArray = [GifView delayArrayWithSource:src];
            ...
            if (image == nil) {
                image = [GifView imageWithSource:src andIndex:weakSelf.frameIndex];
...
+ (NSArray<NSNumber*> *)delayArrayWithSource:(CGImageSourceRef)src {
    ...
    for (size_t i = 0; i < frameCount; i++) {
        NSTimeInterval delay = [GifView delayTimeWithSource:src andIndex:i];
...
+ (NSTimeInterval)delayTimeWithGifData:(NSData *)gifData andIndex:(size_t)index {
    ...
    if (src) {
        NSTimeInterval delay = [GifView delayTimeWithSource:src andIndex:index];
        CFRelease(src);
        return delay;
    }
    else {
...
+ (NSArray<NSNumber*> *)delayArrayWithGifData:(NSData *)gifData {
    CGImageSourceRef src = CGImageSourceCreateWithData((__bridge CFDataRef)gifData, NULL);
    if (src) {
        NSArray *array = [GifView delayArrayWithSource:src];
        CFRelease(src);
        return array;
    }
    else {
...
+ (UIImage *)imageWithGifData:(NSData *)gifData andIndex:(size_t)index {
    CGImageSourceRef src = CGImageSourceCreateWithData((__bridge CFDataRef)gifData, NULL);
    if (src) {
        UIImage *image = [GifView imageWithSource:src andIndex:index];
        CFRelease(src);
        return image;
    }
    else {
...
+ (NSArray<UIImage*> *)imageArrayWithGifData:(NSData *)gifData {
    ...
    if (src) {
        ...
        size_t frameCount = CGImageSourceGetCount(src);
        for (size_t i = 0; i < frameCount; i++) {
            UIImage *image = [GifView imageWithSource:src andIndex:i];
