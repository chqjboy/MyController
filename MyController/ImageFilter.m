...
static  void* bitmapdata = nil;
const float matrix_filter[][20]=
{
    //LOMO
    {
...
static unsigned char *RequestImagePixelDataf(CGImageRef inImage, CGSize size)
// 返回一个指针，该指针指向一个数组，数组中的每四个元素都是图像上的一个像素点的RGBA的数值(0-255)，用无符号的char是因为它正好的取值范围就是0-255
{
    ...
    CGContextDrawImage(cgctx, rect, inImage); //将目标图像绘制到指定的上下文，实际为上下文内的bitmapData。
    unsigned char *data = CGBitmapContextGetData (cgctx);
...
{
    const float* f = matrix_filter[eImageFileter];
...
{
    const float* f = matrix_filter[eImageFileter];
    GLuint w = (GLuint)CGImageGetWidth(inImage);
    GLuint h = (GLuint)CGImageGetHeight(inImage);
    CGSize size = CGSizeMake(w*1.0, h*1.0);
    unsigned char *imgPixel = RequestImagePixelDataf(inImage, size);
    int wOff = 0;
    int pixOff = 0;
