...
@interface FadeCtrler ()

@end

@implementation FadeCtrler
...
-(void)animationAction
{
    if (self.flag)
    {
        [Animations fade:self.imageView andAnimationDuration:2 andAlphaBegin:0 andEndAlpha:1 completion:^(BOOL finished) {
            self.flag = NO;
            [self animationAction];
        }];
    }else
    {
        [Animations fade:self.imageView andAnimationDuration:2 andAlphaBegin:1 andEndAlpha:0 completion:^(BOOL finished) {
            self.flag = YES;
            [self animationAction];
        }];
    }
